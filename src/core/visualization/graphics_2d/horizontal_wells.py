...
class DirectionalProfilesGraphic:
    ...
    def __init__(self, well_profile):
        raise TypeError

    @__init__.register
    def _from_correct(self, well_profile: DirectionalProfile):
        self.well_profile = well_profile

        x = [0.0] + well_profile.dislocations
        y = [0.0] + list(map(lambda x: -x, well_profile.depths))
    ...
    def draw(self):
        ...
        self.ax.set_xlim(-self.well_profile.H - 500, self.well_profile.H + 500)
        self.ax.set_ylim(-self.well_profile.H - 500, 0)
        self.ax.axis('equal')
...
profile = DirectionalProfilesGraphic(TangentialFourInterval(1678, 900, 40, 30, 382, 1900))

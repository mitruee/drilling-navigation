...
from PyQt6.QtWidgets import (
    QApplication, QWidget, QLabel, QPushButton, QRadioButton, QFormLayout, QScrollArea,
    QGroupBox, QLineEdit, QVBoxLayout, QGridLayout, QStackedWidget, QButtonGroup, QDialog, QTableWidget, QTableWidgetItem, QVBoxLayout
)
...
# Методы для обработки переключения радиокнопок
    ...
    def on_confirm_horizontal(self):
        ...
        try:
            ...
            horizontal_profile = self.create_horizontal_profile(idx_horz, vals_horz)
    ...
    def create_horizontal_profile(self, idx, vals):
        """
        ...
        # Получаем параметры направляющей части (три первых: H, A, a)
        idx_nav = self.params_stack.currentIndex()
        if idx_nav == 0:
            nav_vals = [float(le.text()) for le in self.inputs_h1]
        elif idx_nav == 1:
            nav_vals = [float(le.text()) for le in self.inputs_h2]
        elif idx_nav == 2:
            nav_vals = [float(le.text()) for le in self.inputs_h3]
        elif idx_nav == 3:
            nav_vals = [float(le.text()) for le in self.inputs_h4]
        elif idx_nav == 4:
            nav_vals = [float(le.text()) for le in self.inputs_h5]
        else:
            nav_vals = []
